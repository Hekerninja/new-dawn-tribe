...
const SobrietyTracker = () => {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('sobrietyTrackerUser');
    if (savedUser) {
      ...
      // Calculate streak
      const startDate = new Date(parsedUser.startDate);
  ...
  useEffect(() => {
    if (userData) {
      localStorage.setItem('sobrietyTrackerUser', JSON.stringify(userData));
